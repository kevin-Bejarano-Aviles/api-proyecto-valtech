...
module.exports = [ // Export our validations
  ...
  check('age').notEmpty().withMessage('Ingrese su edad').isInt({ min: 18, max: 99 })
  ...
  body('avatar').custom((value, { req }) => {
    ...
    return true;
    // If the user don't add an img will reject the form
    // with the message "tiene que ingresar una imagen"
  }).withMessage('Tiene que ingresar una imagen'),
  body('avatar').custom((value, { req }) => {
    value = req.files[0];
    // Declare what type of img our form will accept
    // and if the user enter a diferent type will reject it
