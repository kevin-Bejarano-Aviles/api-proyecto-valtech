...
const getAllStudent = async (req, res) => {
    try {
        const students = await studentModel.findAll({attributes:[ 'id','fullName','email','phoneNumber','program','avatar','dni','school','age','address','motive','user','eventId','adviserId','createdAt','updatedAt']});
        res.json(students);
    } catch (error) {
        res.status(500).json({error:error.message})
    }
