...
const DurationInput = ({ label, name, areInputVisible, formik, errorCreateEventObject, initialValues, handleAreInputVisible }) => {
  ...
  return (
    <div className='relative flex flex-col gap-1 tablet:grow tablet:max-w-[320px]'>
      ...
      <div className={`${areInputVisible.duration ? 'border-green' : ''} ${selectedDuration === '' ? 'text-lightgray' : 'bg-inputbackground'} flex items-center h-10 rounded-lg cursor-pointer border-2`}>
        <div
          name={name}
          id={name}
